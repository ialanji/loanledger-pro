...
// Test endpoint for checking payment table structure
app.get('/api/test/payments-table', async (req, res) => {
  try {
    // Проверяем структуру таблицы credit_payment
    ...
    const countQuery = `SELECT COUNT(*) as count FROM credit_payment`;
    const countResult = await pool.query(countQuery);
    
    // Получаем несколько примеров записей
    const sampleQuery = `SELECT * FROM credit_payment LIMIT 5`;
    ...
    for (const credit of result.rows) {
      ...
      try {
        ...
        const paidPaymentsQuery = `
          SELECT period_number, status, paid_at
          FROM credit_payment 
