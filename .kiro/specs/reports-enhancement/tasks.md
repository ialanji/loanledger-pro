...
- [ ] 2. Implement backend API enhancements for forecast report
  - [ ] 2.1 Update `/api/reports/forecast` endpoint to accept filter parameters
  ...
  - [ ] 2.2 Apply filters before generating payment schedules
    - Modify credit selection query to include filter conditions
    - Ensure ScheduleEngine receives only filtered credits
    - _Requirements: 6.5_

  - [ ]* 2.3 Add error handling for invalid filter parameters
    - Validate date format (ISO 8601)
    - Validate bankId format (UUID)
    - Return 400 status with error message for invalid inputs
    - _Requirements: 6.7_

- [ ] 3. Implement backend API enhancements for portfolio report
  - [ ] 3.1 Update database query to include credit details
  ...
  - [ ] 3.2 Implement current rate lookup for each credit
  ...
  - [ ] 3.5 Structure response with credit details array
  ...
  - [ ] 3.6 Calculate overall totals across all banks
    - Sum totalPrincipal across all banks
    - Sum totalCredits (count) across all banks
    - Sum totalPaid across all banks
    - _Requirements: 7.7_

  - [ ]* 3.7 Add error handling for database queries
    - Wrap queries in try-catch blocks
    - Return 500 status with error message on failure
    - Log detailed errors to console
    - _Requirements: 7.8_

- [ ] 4. Update frontend Reports component UI
  - [ ] 4.1 Replace "Формат экспорта" selector with "Форма отчета" selector
  ...
  - [ ] 4.2 Add state for expandable bank credits in portfolio report
    - Add `expandedBanks` state: `Record<string, boolean>`
    - Create `toggleBankCredits` function to toggle expansion state
    - _Requirements: 3.1, 3.4_

  - [ ] 4.3 Update filter application to pass parameters to API
...
- [ ] 5. Implement forecast report list view rendering
  - [ ] 5.1 Keep existing table structure for list view
...
- [ ] 6. Implement forecast report table view rendering
  - [ ] 6.1 Create pivot table transformation utility function
  ...
  - [ ] 6.4 Render pivot table body with aggregated data
  ...
  - [ ] 6.5 Calculate and render grand totals footer
  ...
  - [ ] 6.6 Add conditional rendering based on reportForm state
    - Use `reportForm === 'list'` to show list view
    - Use `reportForm === 'table'` to show table view
    - _Requirements: 2.3, 2.4_

- [ ] 7. Implement portfolio report credit details expansion
  - [ ] 7.1 Add "Показать кредиты" button to each bank row
  ...
  - [ ] 7.2 Render expandable credit details table
  ...
  - [ ] 7.3 Format credit detail fields correctly
  ...
  - [ ] 7.4 Sort credits by contract number within each bank
    - Ensure backend returns credits sorted by contract_number
    - Or sort in frontend before rendering
    - _Requirements: 3.6_

- [ ] 8. Update reportsService to handle new data structures
  - [ ] 8.1 Update `PortfolioReportData` interface
    - Add `credits: CreditDetail[]` to items array type
    - _Requirements: 3.3, 7.2_

  - [ ] 8.2 Ensure API calls pass all filter parameters
    - Verify `dateFrom`, `dateTo`, and `bankId` are included in query params
    - _Requirements: 1.5, 4.1_

- [ ] 9. Add validation and error handling in frontend
  - [ ]* 9.1 Validate date range before API call
...
- [ ] 10. Test forecast report with filters
  ...
  - [ ]* 10.2 Test table view rendering
...
- [ ] 11. Test portfolio report enhancements
  - [ ]* 11.1 Test credit details expansion
  ...
  - [ ]* 11.2 Verify remaining balance calculation
  ...
  - [ ]* 11.3 Verify weighted average rate calculation
