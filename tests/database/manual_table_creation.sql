...
-- Разрешаем доступ как аутентифицированным, так и анонимным пользователям
CREATE POLICY "Users can view expense sources" ON expense_sources
  FOR SELECT USING (auth.role() IN ('authenticated', 'anon') OR auth.role() IS NULL);

CREATE POLICY "Users can insert expense sources" ON expense_sources
  FOR INSERT WITH CHECK (auth.role() IN ('authenticated', 'anon') OR auth.role() IS NULL);

CREATE POLICY "Users can update expense sources" ON expense_sources
  FOR UPDATE USING (auth.role() IN ('authenticated', 'anon') OR auth.role() IS NULL);

CREATE POLICY "Users can delete expense sources" ON expense_sources
  FOR DELETE USING (auth.role() IN ('authenticated', 'anon') OR auth.role() IS NULL);
...
(
  'supplies',
  'https://docs.google.com/spreadsheets/d/176DMZtRQEUxBKqp86wP8OlXghTdPUcBc/edit?usp=drive_link&ouid=105171816895029403665&rtpof=true&sd=true',
  'Rechizite',
  '{
    "date": "A",
    "item": "B",
...
(
  'other',
  'https://docs.google.com/spreadsheets/d/1x94N2ccAkYKXBXNAQVGcPP_YTdHiyRpH0I6T414VlHM/edit?usp=drive_link',
  'Cheltuieli',
  '{
    "date": "A",
    "category": "B",
