...
const { calculateDashboardStats, parseNumeric } = require('../../src/lib/dashboardCalculations');
...
describe('Dashboard Calculations', () => {
  ...
  describe('Basic calculation functionality', () => {
    test('should calculate basic stats without schedule or historical data', () => {
      const result = calculateDashboardStats(mockCredits, mockScheduledPayments);
    ...
    test('should handle null and undefined values in data', () => {
      ...
      const result = calculateDashboardStats(creditsWithNulls, paymentsWithNulls);
  ...
  describe('Monthly calculations', () => {
    test('should calculate current month payments correctly', () => {
      ...
      const currentMonthPayments = [
        {
          id: 'current-1',
          creditId: 'credit-1',
          dueDate: new Date(currentYear, currentMonth, 15).toISOString().split('T')[0],
          totalDue: 15000,
          principalDue: 10000,
          interestDue: 5000,
          status: 'scheduled'
      ...
      const result = calculateDashboardStats(mockCredits, currentMonthPayments);

      expect(result.thisMonthDue).toBe(40000); // 15k + 25k
      expect(result.thisMonthPrincipal).toBe(30000); // 10k + 20k
      expect(result.thisMonthInterest).toBe(10000); // 5k + 5k
    });

    test('should handle invalid dates in monthly calculations', () => {
      const paymentsWithInvalidDates = [
        {
          id: 'invalid-1',
          creditId: 'credit-1',
      ...
      const result = calculateDashboardStats(mockCredits, paymentsWithInvalidDates);

      // Should only count valid dates
      expect(result.thisMonthDue).toBe(25000); // Only valid payment counted
    });
  });

  describe('Overdue calculations', () => {
    test('should calculate overdue amounts correctly', () => {
      const paymentsWithOverdue = [
        ...mockScheduledPayments,
        {
          id: 'overdue-1',
          creditId: 'credit-1',
      ...
      const result = calculateDashboardStats(mockCredits, paymentsWithOverdue);

      expect(result.overdueAmount).toBe(30000); // 12k + 18k
    });

    test('should handle no overdue payments', () => {
      const result = calculateDashboardStats(mockCredits, mockScheduledPayments);
  ...
  describe('Data validation and error handling', () => {
    test('should validate all numeric results are non-negative', () => {
      const result = calculateDashboardStats(mockCredits, mockScheduledPayments, mockScheduleData, mockHistoricalPayments);
    ...
    test('should handle corrupted data gracefully', () => {
      ...
      expect(() => {
        calculateDashboardStats(corruptedCredits, corruptedPayments);
      }).not.toThrow();

      const result = calculateDashboardStats(corruptedCredits, corruptedPayments);
  ...
  describe('Projected interest calculation accuracy', () => {
    ...
    test('should maintain calculation consistency across multiple calls', () => {
      const result1 = calculateDashboardStats(mockCredits, mockScheduledPayments, mockScheduleData, mockHistoricalPayments);
      const result2 = calculateDashboardStats(mockCredits, mockScheduledPayments, mockScheduleData, mockHistoricalPayments);
